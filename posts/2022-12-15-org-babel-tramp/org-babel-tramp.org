...
will use this location by default. We set this up using the =#+PROPERTY:= argument:

#+begin_example
#+PROPERTY: header-args:python :dir /ssh:<server>:/path/to/dir
#+end_example

We've added =:dir= that specifies the remote path using tramp. Whenever a python code
block is executed normally with =C-c C-c=, it will connect to =<server>= and navigate to
the =/path/to/dir= directory, start a python process, execute the source code and
return the results.
...
manually change the =org-babel-python-command= to the path of the virtual environment
to use:
...
: plt.close()
: '/ssh:lis.me|ssh:presage1:/tmp/plot.png'
#+end_example
